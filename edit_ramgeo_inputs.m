function outdir = edit_ramgeo_inputs(indir, varargin)
% Inputs:
...
cd(indir)
outdir = strcat(num2str(newFreqHzNum), 'Hz'); 
...
for itr2 = 1:length(envIdx)
    ...
    while ~feof(fid_in)
        ...
        if lineCounter == 1
            ...
            newOutFile = strcat(outdir, '\', site, '_', newFreqHz, 'Hz', '_', char(token(length(token))), '_', 'deg');
            
        elseif lineCounter == 2 && modFreq == true
            [freqSubstr1, remain] = strtok(s, '	');
            [freqSubstrNoDecimal, remain] = strtok(freqSubstr1, '.');
            % enter new frequency, (a little added complexity to make sure other
            % things are not accidentally replaced too)
            strIndex = strfind(s,freqSubstrNoDecimal);
            firstInstance = strIndex(1);
            s = [s(1:firstInstance - 1), newFreqHz, s(firstInstance + length(freqSubstrNoDecimal):end)];
        end
